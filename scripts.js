        ...
        function subscribe(e) {
            if ("#" === $(e).attr("action")) {
                ...
                a = {
                    ...
                    success: function(e) {
                        if ("success" !== e.result) {
                            var a = "Error: try again later";
                            e.msg.search("already subscribed") > -1 ? a = "Already subscribed" : e.msg.search("too many") > -1 ? a = "Too many recent attempts" : e.msg.search("captcha") > -1 ? a = "Error: reCAPTCHA must be disabled" : e.msg.search("enter a value") > -1 ? a = "Error: non-email field set to required" : console.log(e.msg.replace(/(<([^>]+)>(.*?)<([^>]+)>)/, "")),
                            $(".capture-success").html(t(a))
                        }
                        $(".blank-success-message").show()
                    }
                };
...
var btn = document.getElementsByClassName("twitch-chat-open")
  , toggleChat = function() {
    var e = this.previousElementSibling
      , a = this.firstChild
      , n = this.lastChild;
    0 === parseFloat(getComputedStyle(e, null).height.replace("px", "")) ? (a.innerHTML = t("Close Chat"),
    e.style.height = "400px",
    n.style.transform = "rotate(180deg)") : (a.innerHTML = t("Open Chat"),
    e.style.height = "0",
